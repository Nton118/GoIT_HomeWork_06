...
def normalize(file_name:str) -> str:
    ...
    for ch in file_name:
        ...
        elif  ch.isnumeric() or ch.isalpha() or ch == "." or ch == "_" or ch == " ":
            output += ch
        else:
            output += "_"
...
def scan_folder(path:Path):
    ...
    for item in contents:
        ...
        if item.is_file():
            ...
            if item.name not in ('images', 'videos', 'docs', 'music', 'archives', 'apps'):
                scan_folder(item)
            else: 
                continue
...
def main():
    ...
    if archives:
        archives_report = unpack_files(archives, path)
        report_category('Archives', archives_report)
